...
export async function POST() {
  ...
  const res = await fetch(
    `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/actions/workflows/daily-scrape.yml/dispatches`,
    {
      method: 'POST',
      headers: {
  ...
  return NextResponse.json({
    message: 'Scrape job triggered via GitHub Actions. Refresh in ~2 minutes to see results.',
    triggered: true,
  });
}
