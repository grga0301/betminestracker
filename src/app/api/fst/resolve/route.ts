...
export async function POST() {
  ...
  const res = await fetch(
    `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/actions/workflows/daily-resolve-fst.yml/dispatches`,
    {
      method: 'POST',
      headers: {
  ...
  return NextResponse.json({
    message: 'FST resolve triggered via GitHub Actions. Refresh in ~2 minutes.',
    triggered: true,
  });
}
